...
drop function if exists pedido_cliente;
delimiter //
create function pedido_cliente (id int(10))
returns int deterministic
begin
	return (select count(*) from pedido where id_cliente = id);
end //
delimiter ;

select pedido_cliente (2);


-- 2 calcular la comision total ganada por un comercial
drop function if exists comision_total;
delimiter //
create function comision_total (id int(10))
...
-- obtener el cliente con mayor numero de pedidos
drop function if exists comision_total;
delimiter //
create function mayor_cantidad ()
