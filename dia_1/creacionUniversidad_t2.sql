...
select * from asignatura a inner join grado g on a.id_grado = g.id where g.nombre = 'Grado en Ingeniería Informática (Plan 2015)';
select * from alumno;

select al.*, a.nombre as nombre_asignatura from alumno_se_matricula_asignatura am 
inner join asignatura a on am.id_asignatura = a.id 
inner join curso_escolar ce on am.id_curso_escolar = ce.id 
inner join alumno al on am.id_alumno = al.id 
...
select count(*) as cantidad from alumno where year(fecha_nacimiento) = 1999 and sexo='H';
...
select d.nombre , count(*) from profesor inner join departamento d on profesor.id_departamento = d.id group by d.nombre order by 2 desc;
...
select d.nombre , count(profesor.id) from profesor right join departamento d on profesor.id_departamento = d.id group by d.nombre order by 2 desc;

-- Devuelve un listado con el nombre de todos los grados existentes en la base de datos y
-- el número de asignaturas que tiene cada uno.
...
select g.nombre , count(a.nombre) from grado g left join asignatura a on g.id = a.id_grado group by 1 order by 2 desc;
...
select g.nombre , count(a.nombre) from grado g left join asignatura a on g.id = a.id_grado group by 1 having count(a.nombre) > 40;

-- Devuelve un listado que muestre el nombre de los grados
...
select g.nombre, a.tipo, sum(a.creditos) from asignatura a left join grado g on a.id_grado = g.id group by a.tipo, g.nombre order by 3 desc;
...
select ce.anyo_inicio, count(asm.id_alumno) from alumno_se_matricula_asignatura asm inner join curso_escolar ce on asm.id_curso_escolar = ce.id group by ce.anyo_inicio;
...
-- El resultado mostrará cinco columnas:
-- id, nombre, primer apellido, segundo apellido y número de asignaturas.
-- El resultado estará ordenado de mayor a menor por el número de asignaturas.
select p.id, p.nombre, p.apellido1, p.apellido2, count(a.nombre) from profesor p inner join asignatura a on p.id = a.id_profesor group by p.id order by 5 desc;
